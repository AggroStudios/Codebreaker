...
export default class Terminal {
    ...
    private _stdin: (
        ...
        _callback: (char: string) => void,
        _options?: TerminalAttachmentOptions,
    ) => {
    ...
    private _stdout: (
        message: string,
        options?: TerminalAttachmentOptions,
    ) => void = (_message: string, _options?: TerminalAttachmentOptions) => {
...
/_______  /|__| |____/\____ | |__|\____/
    ...
    addHistory(commandLine: string, commandId: number) {
        if (!this.history[0] || this.history[0].command !== commandLine) {
            this._history = [
                { command: commandLine, id: commandId },
                ...this._history.slice(0, this.options.historySize - 1),
            ];
        }
    }
            ...
            this.stdin((char) => {
                switch (char) {
                    ...
                    default:
                        ...
                        buffer = char;
                        this.stdout(
                            `${prompt}${characterOverride ? characterOverride.repeat(char.length) : char}`,
                            { updateMode: true },
                        );
                        break;
                }
    ...
    showLoader(loaderCharacters = ['|', '/', '-', '\\']) {
        if (isNull(this.loaderTimer)) {
            ...
            this.loaderTimer = setInterval(() => {
                ...
                this.loaderChar = loaderCharacters[loaderIndex];
                this.stdout(this.loaderChar, {
                    replaceRange: [-this.loaderChar.length],
                });
            }, 70);
        }
    }
            ...
            this.stdin((char) => {
                switch (char) {
                    case '^C':
                        ...
                        reject([
                            `${previousString}^C`,
                            { replaceRange: [-previousString.length] },
                        ]);
                        break;
                }
            });
